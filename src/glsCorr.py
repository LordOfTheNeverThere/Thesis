...
def getGLSCorr(proteinData: ProteinsMatrix, pValues: bool = True, dataForCovariance:pd.DataFrame = None) -> PairwiseCorrMatrix:
    ...
    if dataForCovariance is not None:
        ...
        samplesInCommon = proteinData.index.intersection(genomicData.index)
    ...
    print(covMatrix)
    print(np.all(np.linalg.eigvals(covMatrix) > 0))
...
if __name__ == '__main__':
    ...
    genomicData = pd.read_csv(PATH + '/datasetsTese/genomics.csv', index_col='Unnamed: 0').transpose()
    glsCoefs = getGLSCorr(proteinData)
